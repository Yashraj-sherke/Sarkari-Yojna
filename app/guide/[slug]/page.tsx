...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const g=guides.find(g=>g.slug===slug);if(!g)return { title: 'Not Found' };return {title:g.title,description:g.description,alternates:{canonical:'/guide/'+slug},openGraph:{title:g.title,description:g.description,url:'/guide/'+slug,type:'article',locale:'hi_IN',images:[DEFAULT_OG_IMAGE]}};}
